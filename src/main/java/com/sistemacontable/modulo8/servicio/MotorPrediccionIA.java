...
public class MotorPrediccionIA {
    ...
    public ResultadoPrediccion predecir(List<RegistroFinanciero> historial,
        ...
        if (ultimo.getTotalVentas() > 0) {
            tendencia = ((ventasPred - ultimo.getTotalVentas()) / ultimo.getTotalVentas()) * 100.0;
        } else if (ventasPred > 0) {
            tendencia = 100.0; // Si ventas anteriores eran 0 y ahora hay ventas, 100% crecimiento
        }
